...
- 搜索阶段保持宽覆盖，正式入池前强制核验当前公司、当前岗位族和明确排除项，避免把搜索命中直接当成合格人脉
- 将公司或岗位证据冲突的人放入“待核验”，将明确不符合的人放入“排除清单”；只有严格合格的人才能参与优先级排序和开场白生成
...
默认的标准扫描会先建立宽覆盖的原始候选层，再通过当前公司、当前岗位族和明确排除项三类硬门槛生成正式联系人池。公司或岗位证据模糊的人进入 `待核验`，明确不符合的人进入 `排除清单`；两者都不会参与高优先级排序。校友、共同好友和资历只用于合格池内的排序，不能挽救硬条件不符的人选。搜索会按公司别名和岗位族记录覆盖路径，根据连续批次的新增合格人脉决定是否收敛。人数只是搜索过窄的预警，不是凑数目标；标准扫描少于 40 人时会自动完成一次扩展复查。只有用户明确要求“快速看看”或给出较小数量上限时，才收窄为小样本。
